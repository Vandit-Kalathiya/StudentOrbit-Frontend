  ...
  return (
    <div className={`${week.weekNumber === 1 ? "rounded rtl xl bg-gray-100" : ""}`}>
      <h2 id={`accordion-open-heading-${week.weekNumber}`}>
        <button
          type="button"
          className={`w-full flex items-center justify-between p-5 font-medium text-black border border-b-0 bg-gray-50 hover:bg-[#5B6DF3] hover:text-white gap-3 text-xl ${week.weekNumber === 1 ? "rounded-t-xl bg-gray-100" : ""
            }`}
          onClick={onToggle}
        ...
        <div className="px-5 pb-5 pt-2 border border-t-0 border-gray-300 bg-gray-50 text-lg">
          {Array.isArray(week.tasks) ? (
            <ul className="ps-5 text-gray-700 list-disc">
              {week.tasks.length == 0 ? <p>No any task yet.</p> :
                week.tasks.map((task) => (
